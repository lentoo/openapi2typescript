...
import Log from './log';
import { stripDot, toCamelCase, writeFile } from './util';
...
class ServiceGenerator {
  ...
  public getInterfaceByTypeName(
    ...
    return interfaces.find((r) => r.typeName === typeName);
  }
  ...
  public traverse(
    ...
    if (type.props && Array.isArray(type.props) && type.props.length > 0) {
      const props = type.props[0];
      const process = (refPath: string) => {
        const refDTO = refPath.replace('[]', '');

        const refDTOType = this.getInterfaceByTypeName(refDTO, interfaceTP);
        if (refDTOType) {
          if (!deepDTO[refDTO]) {
            deepDTO[refDTO] = refDTOType;
            this.traverse(refDTOType, interfaceTP, deepDTO);
          }
        }
      };
  ...
  public genFile() {
    ...
    const prettierError = [];
    // 生成 service 统计
    let serviceTP = this.getServiceTP();
    if (tags.length > 0) {
      const renameTags = tags.map((t) => toCamelCase(t));
    ...
    serviceTP.forEach((service) => {
      ...
      const deepDtos = Object.values(deepDTO);

      let interfaces = [];
      if (deepDtos && deepDtos.length > 0) {
        interfaces = [...serviceInterfaceTP, ...deepDtos];
      } else {
        interfaces = serviceInterfaceTP;
      }
      // 生成 ts 类型声明
      this.genFileFromTemplate('typings.d.ts', 'interface', {
        ...
        // namespace: 'API',
        list: this.sortType(interfaces), // this.getInterfaceTP(),
        disableTypeCheck: false,
        tagsName: service.className,
      });
    });
    ...
    //   nullable: this.config.nullable,
    //   // namespace: 'API',
    //   list: interfaceTP, // this.getInterfaceTP(),
    //   disableTypeCheck: false,
    // });
  ...
  public getInterfaceTP() {
    const { components, paths } = this.openAPIData;
    const data =
      components &&
